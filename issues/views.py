...
class IssueListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ).order_by("created_on").order_by("priority").reverse()

        #Get separated states of issues

        todo = Status.objects.get(name = "to do")
        temp_todo_list = context['issue_list'].filter(
            status = todo
        )        
        ...
        temp_in_progress_list = context['issue_list'].filter(
            status = in_progress
        )
        ...
        temp_done_list = context['issue_list'].filter(
            status = done
        )
...
class IssueUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if form.instance.assignee != issue.assignee and \
            role != self.request.user:
